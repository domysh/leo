...
Per eseguire i test sono state implementate 3 modalità di posizionamento del satellite: 'multiple', 'single', 'single-displocated', che definisco nel primo caso una flotta di satellita come in Figura~\label{fig:multi_car_one_sat_example_nodes}, nel secondo un singolo satellite di fronte all'automobile posizionata con i parametri di default (anche questa impostazione è di default), mentre l'ultimo caso posiziona il satellite in una posizione dislocata rispetto all'automobile, in modo da avere un satellite a 300km di altitudine, inclinato di 20° e con longitudine di 90° e offset di 180°, come mostrato in Figura~\ref{fig:sat_custom_positioning}.

inoltre sono stati implementati diverse modalità di installazione di antenne: IsotropicAntennaModel, CircularApertureAntennaModel, ParabolicAntennaModel, ThreeGppAntennaModel, e CosineAntennaModel. Di seguito si mostrerà come si comportano le antenne in queste diverse configurazioni.

\subsubsection{CircularApertureAntennaModel}

\textit{--duration=2s --mobilityPrecision=1s --traceFile=track.csv --satMode=single --antennaModel=CircularApertureAntennaModel --txPower=120 --logging}: con questi parametri riusciamo a trasmettere i pacchetti senza perdite, ma già a 100dB non ci è possibile inviare i pacchetti

Però durante i test si delinea un comportamento anomalo:

\textit{--duration=2s --mobilityPrecision=1s --traceFile=track.csv --satMode=single --antennaModel=CircularApertureAntennaModel --txPower=110 --logging}: qui non riusciamo a trasmettere i pacchetti.
\textit{--duration=2s --mobilityPrecision=1s --traceFile=track.csv --satMode=single-dislocated --antennaModel=CircularApertureAntennaModel --txPower=110 --logging}: qui invece 8/10 pacchetti vengono ricevuti correttamente, ma con un SINR molto basso

Nell'esempio in cui i pacchetti arrivano in posizionamento diretto abbiamo SINR che oscilla tra -19 e -21, mentre latenze che oscillano tra i 19 e 20ms.

\subsubsection{ParabolicAntennaModel}

\textit{--duration=2s --mobilityPrecision=1s --traceFile=track.csv --satMode=single --antennaModel=ParabolicAntennaModel --txPower=70 --logging}: con questi parametri riusciamo a trasmettere i pacchetti senza perdite, con SINR tra i -18 e i -20, e latenze sempre tra i 19 e i 20ms.

Con le stesse opzioni ma in modalità displocata non abbiamo la trasmissione dei pacchetti, con un SINR sui -47dB.

\subsubsection{ThreeGppAntennaModel}

\textit{--duration=2s --mobilityPrecision=1s --traceFile=track.csv --satMode=single --antennaModel=ThreeGppAntennaModel --txPower=70 --loggingg}: con questi parametri riusciamo a trasmettere i pacchetti senza perdite, con -19dB di SINR e latenze tra i 19 e i 20ms.

\textit{--duration=2s --mobilityPrecision=1s --traceFile=track.csv --satMode=single-dislocated --antennaModel=ThreeGppAntennaModel --txPower=95 --logging}: con questi parametri riusciamo a trasmettere i pacchetti senza perdite, con SINR a -18db e latenze sui 18ms.

A 95dB in direzione diretta abbiamo -15 di SINR e 3 ms di latenza, con 10 pacchetti ricevuti correttamente.

\subsubsection{CosineAntennaModel}

\textit{--duration=2s --mobilityPrecision=1s --traceFile=track.csv --satMode=single --antennaModel=CosineAntennaModel --txPower=0.1 --logging}

Con questa configurazione, con txPower a 0.1, riusciamo a comunicare correttamente con SINR a 5dB e latenze sui 3ms, mentre con la stessa con 13/14 dB di SINR e latenze sui 3ms. Mentre con posizioni displocate i pacchetti non vengono tutti ricevuti correttamente, il SINR passa a -20/-22dB.
...
\begin{lstlisting}[
    ...
    basicstyle=\footnotesize\ttfamily,
    breaklines=true,
    caption={Correzione del bug: Aggiunta di un controllo per nodi in posizione identica per evitare NaN generati da divisione per 0},
    label={lst:ns3-elevation-angle-fix}
]
...
index 282feeeda..eef215446 100644
...
@@ -202,6 +202,7 @@ GeocentricConstantPositionMobilityModel::DoGetElevationAngle(
     Vector bMinusA = b - a;
     double numerator = std::abs(a * bMinusA);
     double denominator = a.GetLength() * bMinusA.GetLength();
+    if (denominator == 0) return 0.0; // Distance is zero, no elevation angle
